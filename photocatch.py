...
class Crawler:
    ...
    def __save_image(self, rsp_data, word):
        ...
        if not os.path.exists("./" + word):
            os.mkdir("./" + word)
        # 判断名字是否重复，获取图片长度
        __counter = len(os.listdir('./')) + 1
        for image_info in rsp_data['imgs']:
            try:
                time.sleep(self.time_sleep)
                print image_info['objURL']
                nn=(str)(__counter)+".jpg";
                fp=open(nn, 'a');
                urllib.urlretrieve(image_info['objURL'], nn)
                ...
                print "小黄图+1,已有" + str(__counter) + "张小黄图"
                __counter += 1
        return
    ...
    def __get_images(self, word):
        ...
        while 1==1:
            ...
            try:
                ...
                rsp = page.content.decode('unicode_escape')
